...
VEX Toss Up featured two objects to be scored: *large balls*, weighted beach balls; and *buckyballs*, plastic truncated icosahedrons (the shape of the buckyball molecule, C<sub>60</sub>.) For efficient scoring, our robot carried three buckyballs (the maximum allowed), picking and dispensing them using a modified conveyor belt with flexible flaps. The intake system, as it was called, needed to have zero rigid parts to handle the oddly-shaped balls. The front of the belt was supported by a springloaded arm which pressed on the balls to prevent them from falling out, while allowing the balls to enter and leave from many angles.
